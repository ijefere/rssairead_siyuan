...
let taskID = [];
function scheduleCronJob(cronExpression, taskFunction) {
  ...
  function checkAndRunTask() {
    ...
    if (checkField(minute, currentMinute) && checkField(hour, currentHour) && checkField(dayOfMonth, currentDayOfMonth) && checkField(month, currentMonth) && checkField(dayOfWeek, currentDayOfWeek)) {
      taskFunction();
    }
  }
  function checkField(fieldValue, currentValue) {
    return fieldValue === "*" || fieldValue.includes(currentValue);
  }
  const id = setInterval(checkAndRunTask, 6e4);
  taskID.push(id);
}
function removeAllCronJob() {
  ...
  return siyuan.fetchSyncPost("/api/av/getAttributeViewKeys", {
    ...
    return r.map((av) => {
      return av.keyValues.map((kv) => {
        ...
        return {
          key: kv.key.name,
          value: (_b = (_a = kv.values.find((v) => {
            var _a2;
            return (_a2 = v.text) == null ? void 0 : _a2.content;
          })) == null ? void 0 : _a.text) == null ? void 0 : _b.content,
          avName: av.avName
        };
      });
    }).flat().reduce((pre2, cur) => {
      pre2[cur.key] = (cur.avName.includes("feed") ? cur.value : pre2[cur.key]) ?? cur.value ?? "";
      return pre2;
    }, {});
  });
}
    ...
    XMLNode$1.exports = (function() {
      ...
      XMLNode2.prototype.compareDocumentPosition = function(other) {
        ...
        if (ref === other) {
          ...
          res2 = DocumentPosition2.Disconnected | DocumentPosition2.ImplementationSpecific;
          if (Math.random() < 0.5) {
            res2 |= DocumentPosition2.Preceding;
          } else {
            res2 |= DocumentPosition2.Following;
          }
          return res2;
    ...
    (function(sax2) {
      ...
      SAXStream.prototype.write = function(data) {
        if (typeof Buffer === "function" && typeof Buffer.isBuffer === "function" && Buffer.isBuffer(data)) {
          if (!this._decoder) {
            var SD = require$$3.StringDecoder;
...
function requireParser$1() {
  ...
  (function(exports$1) {
    (function() {
      ...
      setImmediate = require$$3.setImmediate;
      defaults2 = requireDefaults().defaults;
    ...
    parseURL(feedUrl, callback, redirectCount = 0) {
      ...
      let prom = new Promise((resolve, reject) => {
        ...
        let req = get(requestOpts, (res2) => {
          ...
          let encoding = utils2.getEncodingFromContentType(res2.headers["content-type"]);
...
async function parseFeed(feedDoc) {
  var _a;
  let timeout = Number(feedDoc.getAttr("timeout"));
  timeout = timeout >= 3e3 ? timeout : FETCH_TIMEOUT;
  const url = (_a = feedDoc.getAttr("feed")) == null ? void 0 : _a.trim();
  const resText = await new Promise((r, j) => {
    siyuan.fetchPost(
      ...
      (res2) => {
        if (res2.code !== 0 && res2.data.status !== 200) {
          j(new Error(res2.msg));
        } else {
          r(res2.data.body);
        }
      }
  ...
  if (customCode) {
    const customParseFun = eval(customCode);
    const res = await customParseFun(feedDoc, resText, { xssDefend, elText });
    return res;
  }
  try {
    const parser2 = new Parser();
    const feed = await parser2.parseString(resText);
    return {
      title: feed.title || "",
      subtitle: feed.description || "",
      updated: feed.lastBuildDate || (/* @__PURE__ */ new Date()).toISOString(),
      entryList: feed.items.map((item) => ({
        title: item.title,
        published: item.pubDate || item.isoDate,
        updated: item.isoDate || item.pubDate,
        summary: xssDefend(item.contentSnippet || item.content || item.summary),
        link: item.link
      }))
    };
  } catch (e) {
    console.error("RSS Parsing Error with rss-parser:", e);
    return new Error(
      `RSS 解析失败 (rss-parser)，可以将此消息发送给开发者 2550792838@qq.com (rssairead_siyuan): ${url}. Error: ${e}`
    );
  }
}
async function parseFeedBlock(block_id) {
  var _a;
  const feedObj = {
    attr: {},
    entryBlock: [],
    ...
    getAttr(key) {
      var _a2;
      return ((_a2 = this.attr[key]) == null ? void 0 : _a2.value) ?? this.av_attr[key];
    }
  };
  const feedAttrBlock = (_a = (await sqlQuery(
    `SELECT * FROM blocks WHERE parent_id="${block_id}" and fcontent LIKE 'feed:%' limit 1`
  )).data) == null ? void 0 : _a[0];
  if (feedAttrBlock) {
    feedObj.attrBlock = feedAttrBlock;
    const feedAttrChildBlock = (await sqlQuery(`SELECT * FROM blocks WHERE parent_id="${feedAttrBlock == null ? void 0 : feedAttrBlock.id}" limit 20`)).data;
    Object.assign(feedObj.attr, blocksToObj(feedAttrChildBlock));
  }
  feedObj.entryBlock = (await sqlQuery(
    `SELECT * FROM blocks
      WHERE
       parent_id="${block_id}" AND (markdown LIKE "* [ ] #%" OR markdown LIKE "* [X] #%")
      ORDER BY created DESC
      LIMIT ${/** 避免笔记本中存在但没搜到，导致重复插入 */
    MAX_FEED_NUM * 3}`
  )).data;
  Object.assign(feedObj.av_attr, await get_av_map(block_id));
  return feedObj;
  function blocksToObj(blocks) {
    const map2 = blocks.map(
      (el2) => [
        Array.from(el2.content.trim().match(/(.*?):([\s\S]+)/) ?? []),
        el2
      ]
    ).filter((el2) => el2[0].length === 3);
    const obj = {};
    for (const [[_rawContent, key, value], block2] of map2) {
      let v = value;
      if (key === "feed") {
        v = value.replace(
          /** 思源会转义下划线 */
      ...
      obj[key] = { value: v, block: block2 };
    }
    return obj;
  }
}
async function getAllFeedBlocks() {
  return (await sqlQuery(`SELECT * FROM attributes WHERE name="bookmark" and value="feed"`)).data;
}
function elText(el2, selectors) {
  var _a;
  return xssDefend((_a = el2.querySelector(selectors)) == null ? void 0 : _a.textContent);
}
function xssDefend(s) {
  if (!s) return "";
  let filteredStr = (new DOMParser().parseFromString(s, "text/html").documentElement.textContent ?? "").replace(
    /** 过滤特殊标记符 */
    /[<>\[\]\n]/g,
    ""
  );
...
async function generateSummary(text, config) {
  ...
  try {
    ...
    return ((_c = (_b = (_a = data.choices) == null ? void 0 : _a[0]) == null ? void 0 : _b.message) == null ? void 0 : _c.content) || "No summary generated.";
  } catch (error) {
    console.error("LLM Request Failed:", error);
    throw error;
  }
}
async function playText(text, config, apiKey) {
...
async function playOpenAITTS(text, config, apiKey) {
  try {
...
async function ensureCategoryDoc(rootDocId, category) {
...
function parseOpml(xmlContent) {
  ...
  const doc = parser2.parseFromString(xmlContent, "text/xml");
  const outlines = [];
  ...
  if (node.children.length > 0) {
    ...
    Array.from(node.children).forEach((child) => {
      ...
      if (child.tagName.toLowerCase() === "outline") {
        (_a = item.outlines) == null ? void 0 : _a.push(parseOutline(child, currentCategory));
      }
    });
  }
  return item;
}
function flattenOpml(outlines) {
  const result = [];
...
class RssDock {
  ...
  async render() {
    ...
    this.element.innerHTML = `
        ...
        `;
    (_a = this.element.querySelector("#dock-refresh-btn")) == null ? void 0 : _a.addEventListener("click", () => {
      siyuan.fetchPost("/api/notification/pushMsg", { msg: "Refreshing list..." });
      this.render();
    });
    (_b = this.element.querySelector("#dock-fetch-btn")) == null ? void 0 : _b.addEventListener("click", async () => {
      siyuan.fetchPost("/api/notification/pushMsg", { msg: "Starting fetch all..." });
      try {
        await this.plugin.registerAllFeed();
        this.plugin._feedFetch.forEach((f) => f());
      } catch (e) {
        console.error(e);
      }
    });
    (_c = this.element.querySelector("#dock-summary-btn")) == null ? void 0 : _c.addEventListener("click", () => {
      this.plugin.generateAndSaveSummary();
    });
    (_d = this.element.querySelector("#dock-add-btn")) == null ? void 0 : _d.addEventListener("click", async () => {
      const url2 = this.element.querySelector("#dock-url").value;
      const cat = this.element.querySelector("#dock-cat").value;
      const pid = this.element.querySelector("#dock-pid").value;
      if (!url2 || !pid) {
        siyuan.fetchPost("/api/notification/pushMsg", { msg: "URL and Parent ID required" });
        return;
      }
      try {
        siyuan.fetchPost("/api/notification/pushMsg", { msg: "Adding feed..." });
        const catName = cat || "Uncategorized";
        const targetDocId = await ensureCategoryDoc(pid, catName);
        await createFeedBlock(targetDocId, url2, catName);
        siyuan.fetchPost("/api/notification/pushMsg", { msg: "Feed added successfully" });
        this.element.querySelector("#dock-url").value = "";
        this.render();
      } catch (e) {
        console.error(e);
        siyuan.fetchPost("/api/notification/pushMsg", { msg: "Error adding feed: " + e });
      }
    });
    this.element.querySelectorAll(".dock-del-btn").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        const item = e.target.closest(".rss-dock-item");
        const id = item.dataset.id;
        if (id && confirm("Are you sure you want to delete this feed?")) {
          try {
            await deleteFeedBlock(id);
            siyuan.fetchPost("/api/notification/pushMsg", { msg: "Feed deleted" });
            this.render();
          } catch (e2) {
            console.error(e2);
            siyuan.fetchPost("/api/notification/pushMsg", { msg: "Error deleting feed: " + e2 });
          }
        }
      });
    });
    this.element.querySelectorAll(".dock-fetch-single-btn").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        const id = item.dataset.id;
        if (id) {
          siyuan.fetchPost("/api/notification/pushMsg", { msg: "Fetching single feed..." });
          try {
            await this.plugin.feedFetch(id);
          } catch (e2) {
            console.error(e2);
            siyuan.fetchPost("/api/notification/pushMsg", { msg: "Error fetching feed: " + e2 });
          }
        }
      });
    });
...
class RssAiReadPlugin extends siyuan.Plugin {
  constructor() {
    super(...arguments);
    this._feedFetch = [];
  ...
  async openSetting() {
               ...
               <div id="tab-config" class="rss-tab-content active">
                   ...
                   <div class="rss-setting-group">
                      ...
                      <div class="rss-setting-item">
                          ...
                          <input type="number" id="tts-speed" class="rss-input" placeholder="1.0" value="${this.config.tts.speed || 1}" step="0.1" min="0.25" max="4.0">
                      </div>
                   </div>
    ...
    tabs.forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        (_a = dialog.element.querySelector(`#tab-${tab.dataset.tab}`)) == null ? void 0 : _a.classList.add("active");
      });
    });
    const saveBtn = dialog.element.querySelector("#save-config-btn");
    if (saveBtn) {
      saveBtn.addEventListener("click", () => {
        this.config.llm.enable = dialog.element.querySelector("#llm-enable").checked;
        this.config.llm.apiKey = dialog.element.querySelector("#llm-apikey").value;
        this.config.llm.baseUrl = dialog.element.querySelector("#llm-baseurl").value;
        this.config.llm.model = dialog.element.querySelector("#llm-model").value;
        this.config.llm.prompt = dialog.element.querySelector("#llm-prompt").value;
        this.config.summary.enable = dialog.element.querySelector("#summary-enable").checked;
        this.config.summary.cron = dialog.element.querySelector("#summary-cron").value;
        this.config.summary.targetDocId = dialog.element.querySelector("#summary-target").value;
        this.config.tts.enable = dialog.element.querySelector("#tts-enable").checked;
        this.config.tts.provider = dialog.element.querySelector("#tts-provider").value;
        this.config.tts.voice = dialog.element.querySelector("#tts-voice").value;
        this.config.tts.speed = parseFloat(dialog.element.querySelector("#tts-speed").value) || 1;
    ...
    addFeedBtn == null ? void 0 : addFeedBtn.addEventListener("click", async () => {
      const url2 = dialog.element.querySelector("#new-feed-url").value;
      const cat = dialog.element.querySelector("#new-feed-cat").value;
      const parentId = dialog.element.querySelector("#new-feed-parent-id").value;
      if (!url2 || !parentId) {
        siyuan.fetchPost("/api/notification/pushMsg", { msg: "URL 和 父文档 ID 必填" });
        return;
      }
      try {
        siyuan.fetchPost("/api/notification/pushMsg", { msg: "Adding feed..." });
    ...
    importOpmlBtn == null ? void 0 : importOpmlBtn.addEventListener("click", async () => {
      const content = dialog.element.querySelector("#opml-content").value;
      const parentId = dialog.element.querySelector("#opml-parent-id").value;
      if (!content || !parentId) {
        siyuan.fetchPost("/api/notification/pushMsg", { msg: "OPML 内容 和 父文档 ID 必填" });
        return;
      }
      try {
        siyuan.fetchPost("/api/notification/pushMsg", { msg: "Parsing OPML..." });
        const outlines = parseOpml(content);
        const flatFeeds = flattenOpml(outlines);
        siyuan.fetchPost("/api/notification/pushMsg", { msg: `Found ${flatFeeds.length} feeds. Importing...` });
        for (const feed of flatFeeds) {
          const catName = feed.category || "Uncategorized";
          const targetDocId = await ensureCategoryDoc(parentId, catName);
          await createFeedBlock(targetDocId, feed.url, catName);
        }
        siyuan.fetchPost("/api/notification/pushMsg", { msg: "OPML Import complete!" });
        dialog.destroy();
        this.registerAllFeed();
      } catch (e) {
        console.error(e);
        siyuan.fetchPost("/api/notification/pushMsg", { msg: "Error importing OPML: " + e });
      }
    });
  ...
  async registerAllFeed() {
    ...
    const feedBlocks = await getAllFeedBlocks();
    return Promise.all(
      feedBlocks.map(async (block2) => {
        const feedDoc2 = await parseFeedBlock(block2.block_id);
        if (feedDoc2.getAttr("feed")) {
          const cron = feedDoc2.getAttr("cron") || DEFAULT_CRON;
          console.log(`注册 cron job 表达式:${cron} by ${feedDoc2.getAttr("feed")}`, feedDoc2);
          const feedFetch = async () => {
            this.feedFetch(block2.block_id);
          };
          scheduleCronJob(cron, feedFetch);
          this._feedFetch.push(feedFetch);
        } else {
          console.log(feedDoc2, "没有读取到 feed 属性，请对照文档进行设定 feed");
        }
      })
    );
  }
  async feedFetch(feedId) {
    var _a;
    const feedDoc2 = await parseFeedBlock(feedId);
    const feed = await parseFeed(feedDoc2);
    if (feed instanceof Error) {
      throw feed;
    }
    const insertEntry = feed.entryList.sort((a, b) => {
      return Number(b.updated) - Number(a.updated);
    }).filter((el2) => el2.link).filter((el2) => {
      const s = !feedDoc2.entryBlock.find(
        /** 如果entryBlock 的第一行存在当前 entry 的链接就当他俩是同一个 entry
         * TODO 如果有更新的话应该也要再次处理
         */
        (entryBlock) => el2.link && entryBlock.content.split("\n")[0].includes(linkFilter(el2.link))
      );
      return s;
    });
    const msg = `feed:${feedDoc2.getAttr("feed")} 共 ${feed.entryList.length} 条，新增 ${insertEntry.length} 条`;
    console.log(msg);
    siyuan.fetchPost("/api/notification/pushMsg", {
      msg
    });
    const feedBlockId = ((_a = feedDoc2.attrBlock) == null ? void 0 : _a.id) ?? feedId;
    const block2 = await getBlockByID(feedBlockId);
...
`;
  ...
  async generateAndSaveSummary() {
    if (!this.config.llm.enable) {
      siyuan.fetchPost("/api/notification/pushMsg", { msg: "LLM not enabled. Cannot generate summary." });
      return;
    }
    const feedBlocks = await getAllFeedBlocks();
    const groupedFeeds = {};
    for (const block2 of feedBlocks) {
      const feedDoc2 = await parseFeedBlock(block2.block_id);
      const category = feedDoc2.getAttr("category") || "Uncategorized";
      const feed = await parseFeed(feedDoc2);
...
`;
      ...
      try {
        siyuan.fetchPost("/api/notification/pushMsg", { msg: `Generating summary for ${category}...` });
        const summary = await generateSummary(promptText, this.config.llm);
        const targetId = this.config.summary.targetDocId;
        if (targetId) {
          const content = `## ${category} Summary (${(/* @__PURE__ */ new Date()).toLocaleDateString()})
...
`;
          ...
          siyuan.fetchPost("/api/notification/pushMsg", { msg: `Summary generated for ${category} but no target doc ID set.` });
        }
        ...
        console.error(e);
        siyuan.fetchPost("/api/notification/pushMsg", { msg: `Error generating summary for ${category}` });
      }
    }
    siyuan.fetchPost("/api/notification/pushMsg", { msg: "Summary generation complete." });
  }
  async onunload() {
    removeAllCronJob();
  }
}
exports.default = RssAiReadPlugin;
