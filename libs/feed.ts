...
interface feed {
  attr: {
    ...
    /** 领域分类 */
    category?: { value: string; block: block };
  };
  attrBlock?: block;
  /** 已经加载的笔记 */
  entryBlock: block[];
...
export async function parseFeed(feedDoc: feed): Promise<feedByUrl | Error> {
  ...
  try {
      ...
      const feed = await parser.parseString(resText);
      ...
      // The previous implementation had manual DOM parsing. 
      // rss-parser is quite robust, but if it fails, we might want to know why.
