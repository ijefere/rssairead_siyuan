...
function createDocWithMd(notebook, path, markdown) {
  return siyuan.fetchSyncPost("/api/filetree/createDocWithMd", {
    notebook,
    path,
    markdown
  });
...
async function parseFeedBlock(block_id) {
  ...
  const feedAttrBlock = (await sqlQuery(
    `SELECT * FROM blocks WHERE parent_id="${block_id}" and fcontent LIKE 'feed:%' limit 1`
  )).data?.[0];
  if (feedAttrBlock) {
    feedObj.attrBlock = feedAttrBlock;
    const feedAttrChildBlock = (await sqlQuery(`SELECT * FROM blocks WHERE parent_id="${feedAttrBlock?.id}" limit 20`)).data;
...
function elText(el2, selectors) {
  return xssDefend(el2.querySelector(selectors)?.textContent);
}
...
class RssAiReadPlugin extends siyuan.Plugin {
  ...
  async openSetting() {
    ...
    tabs.forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        dialog.element.querySelector(`#tab-${tab.dataset.tab}`)?.classList.add("active");
      });
    });
  ...
  async feedFetch(feedId) {
    ...
    const feedBlockId = feedDoc2.attrBlock?.id ?? feedId;
